...
class _SettingsScreenState extends State<SettingsScreen>
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              config.otaUrl.isEmpty ? '根据 WebSocket 地址自动推导' : config.otaUrl,
              style: const TextStyle(fontSize: 14),
            ),
          ],
        ),
  ...
  void _showAddXiaozhiConfigDialog() {
    ...
    final autoAuthNotifier = ValueNotifier<bool>(false);
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: '留空时根据 WebSocket 地址自动推导',
                          hintStyle: TextStyle(color: Colors.grey.shade400),
                          contentPadding: const EdgeInsets.symmetric(
                            horizontal: 16,
                            vertical: 14,
                    ...
                    const SizedBox(height: 4),
                    const Text(
                      '例如：http://192.168.1.10:8002/xiaozhi/ota/',
                    ...
                    ValueListenableBuilder<bool>(
                      valueListenable: autoAuthNotifier,
                      builder:
                          (context, enabled, _) => SwitchListTile(
                            ...
                            subtitle: const Text('开启后连接前自动调用 OTA 获取动态 token'),
                            value: enabled,
                            onChanged:
                                (value) => autoAuthNotifier.value = value,
                          ),
                    ),
                    const SizedBox(height: 24),
                    ElevatedButton(
                      onPressed: () {
                        ...
                        final enableAutoAuth = autoAuthNotifier.value;

                        if (name.isEmpty ||
                            (!enableAutoAuth && websocketUrl.isEmpty)) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
  ...
  void _showEditXiaozhiConfigDialog(XiaozhiConfig config) {
    ...
    final autoAuthNotifier = ValueNotifier<bool>(config.enableAutoAuth);
